    ...
    <ol>
      ...
      <li>计算 source_type = 0/1/2/3/5/6 的记录，使用了新的估算算法</li>
        <ul>
          ...
          <li>通过百度获取五大门户新闻频道的收录数（使用 site 限定，如 site:news.qq.com），分别除以第 2 点提到的文章数，得到的比值后都以 20% 的权重相加后得出综合比值 D = 5796，收录数平均值 R = 18110579；</li>
          <li>由于文章规模越大的网站通常更为热门，其页面被搜索引擎收录的几率越高，因此不能将 D 直接套用到所有网站上，分别取目标网站百度收录数 Rx 和 五大门户新闻频道平均收录数 R 的自然对数，二者相除得到换算系数 Fx，即 Fx = ln(Rx) / ln(R)，其中 ln(R) 是常数（≈ 16.712），则适用于目标网站的比值 Dx = Fx * D；</li>
